...
from tidal_dl.aigpy.LockHelper import (
    RWLock,
)
from tidal_dl.aigpy.fileHelper import (
    createEmptyFile,
)
...
from tidal_dl.aigpy.progressHelper import (
    ProgressTool,
)
from tidal_dl.aigpy.threadHelper import (
    ThreadTool,
)
...
class DownloadTool(object):
    ...
    def __getMoreUrlsParts__(
        self, urls
    ):
        fileSize, urlSizes = (
            self.__getUrlsSize__(urls)
        )
        if fileSize <= 0:
            return (
                0,
                [],
                "Get some file sizes failed.",
            )
    ...
    def start(
        ...
        if size == 1:
            fileSize, parts, msg = (
                self.__getOneUrlParts__(
                    self.fileUrls[0]
                )
            )
        elif size > 1:
            fileSize, parts, msg = (
                self.__getMoreUrlsParts__(
                    self.fileUrls
                )
            )
        else:
