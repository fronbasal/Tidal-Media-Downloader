...
def display_table(title: str, data: dict) -> None:
    ...
    for key, value in data.items():
        key_str = str(key)[:18]  # Truncate long keys
        value_str = str(value)[:27]  # Truncate long values
        print(f"| {key_str:<18} | {value_str:<27} |")
    
    print(f"+{'-' * 20}+{'-' * 29}+")


def display_track_info(track, stream) -> None:
    """Display track information in a table format."""
    data = {
        "Title": track.title,
        "ID": track.id,
        "Album": track.album.title if track.album else "N/A",
        "Version": track.version or "None",
        "Explicit": str(track.explicit),
        "Max-Q": getattr(stream, 'maxQuality', 'N/A'),
        "Get-Q": getattr(stream, 'quality', 'N/A'),
        "Get-Codec": getattr(stream, 'codec', 'N/A'),
    }
    display_table("TRACK-PROPERTY", data)
...
def display_album_info(album) -> None:
    ...
    data = {
        ...
        "Version": album.version or "None",
        "Explicit": str(album.explicit),
    }
    display_table("ALBUM-PROPERTY", data)
