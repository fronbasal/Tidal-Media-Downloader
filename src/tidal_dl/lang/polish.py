...
class LangPolish(object):
    ...
    SETTING_ALBUMID_BEFORE_FOLDER = (
        "Id przed folderem albumu"
    )
    SETTING_INCLUDE_EP = (
        "Uwzględnij single i EP"
    )
    SETTING_SAVE_COVERS = (
        "Zapisz okładki"
    )
    ...
    SETTING_SAVE_ALBUMINFO = (
        "Zapisz AlbumInfo.txt"
    )
    SETTING_DOWNLOAD_VIDEOS = (
        "Download videos"
    )
    SETTING_ADD_LYRICS = (
        "Dodaj teksty utworów"
    )
    SETTING_LYRICS_SERVER_PROXY = (
        "Serwer proxy dla tekstów"
    )
