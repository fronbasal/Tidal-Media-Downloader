...
class LangUkrainian(object):
    ...
    SETTING_ONLY_M4A = (
        "Перетворювати mp4 на m4a"
    )
    SETTING_ADD_EXPLICIT_TAG = (
        "Додавати тег Нецензурно"
    )
    SETTING_ADD_HYPHEN = (
        "Додавати дефіс"
    )
    ...
    SETTING_SHOW_TRACKINFO = (
        "Показувати інформацію про трек"
    )
    SETTING_SAVE_ALBUMINFO = (
        "Зберігати AlbumInfo.txt"
    )
    SETTING_DOWNLOAD_VIDEOS = (
        "Download videos"
    )
    SETTING_ADD_LYRICS = (
        "Додавати тексти пісень"
    )
