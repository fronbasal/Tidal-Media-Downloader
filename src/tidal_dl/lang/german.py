...
class LangGerman(object):
    ...
    SETTING_ONLY_M4A = (
        "mp4 in m4a konvertieren"
    )
    SETTING_ADD_EXPLICIT_TAG = (
        "Explicit Tag hinzufügen"
    )
    SETTING_ADD_HYPHEN = (
    ...
    SETTING_ARTIST_BEFORE_TITLE = (
        "Künstlername vor Songtitel"
    )
    SETTING_ALBUMID_BEFORE_FOLDER = (
        "ID vor Album-Ordner"
    )
    SETTING_INCLUDE_EP = (
        "Singles & EPs einschließen"
    )
    SETTING_SAVE_COVERS = (
        "Cover speichern"
    )
    ...
    SETTING_SAVE_ALBUMINFO = (
        "AlbumInfo.txt speichern"
    )
    SETTING_DOWNLOAD_VIDEOS = (
        "Download videos"
    )
    SETTING_ADD_LYRICS = (
        "Songtexte hinzufügen"
    )
    SETTING_LYRICS_SERVER_PROXY = (
        "Songtext Proxy-Server"
    )
    ...
    MODEL_ARTIST_PROPERTY = (
        "KÜNSTLER-EIGENSCHAFT"
    )
    MODEL_PLAYLIST_PROPERTY = (
        "PLAYLIST-EIGENSCHAFT"
    )
