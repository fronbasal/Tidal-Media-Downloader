...
class LangPortuguese(object):
    ...
    SETTING_ONLY_M4A = (
        "Converter mp4 para m4a"
    )
    SETTING_ADD_EXPLICIT_TAG = (
        "Adicionar etiqueta explícito"
    )
    SETTING_ADD_HYPHEN = (
        "Adicionar hífen"
    )
    ...
    SETTING_MULITHREAD_DOWNLOAD = (
        "Download de vários tópicos"
    )
    SETTING_ALBUM_FOLDER_FORMAT = (
        "Formato da pasta do álbum"
    )
    ...
    SETTING_SHOW_TRACKINFO = (
        "Mostrar informações da faixa"
    )
    SETTING_SAVE_ALBUMINFO = (
        "Salvar AlbumInfo.txt"
    )
    SETTING_DOWNLOAD_VIDEOS = (
        "Download videos"
    )
    SETTING_ADD_LYRICS = (
        "Adicionar Letra da Música"
    )
    SETTING_LYRICS_SERVER_PROXY = (
        "Proxy do servidor de letras"
    )
