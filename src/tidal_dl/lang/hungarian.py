...
class LangHungarian(object):
    ...
    SETTING_ONLY_M4A = (
        "mp4 konvertálása m4a-ra"
    )
    SETTING_ADD_EXPLICIT_TAG = (
        "Explicit tag hozzáadása"
    )
    SETTING_ADD_HYPHEN = (
        "Kötőjel hozzáadása"
    )
    ...
    SETTING_SHOW_TRACKINFO = (
        "Track infók megjelenítése"
    )
    SETTING_SAVE_ALBUMINFO = (
        "AlbumInfo.txt mentése"
    )
    ...
    CHOICE_ENTER_URLID = (
        "Adja meg az 'Url/ID'-t:"
    )
    CHOICE_EXIT = "Kilépés"
    CHOICE_LOGIN = (
        "AccessToken ellenőrzése"
    )
    CHOICE_SETTINGS = "Beállítások"
    CHOICE_SET_ACCESS_TOKEN = (
        "AccessToken beállítása"
    )
    CHOICE_DOWNLOAD_BY_URL = (
        "Letöltés URL vagy ID alapján"
    )
