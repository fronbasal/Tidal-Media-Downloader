...
class LangDutch(object):
    ...
    SETTING_ONLY_M4A = (
        "mp4 naar m4a converteren"
    )
    SETTING_ADD_EXPLICIT_TAG = (
        "Expliciete tag toevoegen"
    )
    SETTING_ADD_HYPHEN = (
        "Koppelteken toevoegen"
    )
    SETTING_ADD_YEAR = (
        "Jaar voor albummap toevoegen"
    )
    ...
    SETTING_ARTIST_BEFORE_TITLE = (
        "Artiestnaam voor tracktitel"
    )
    SETTING_ALBUMID_BEFORE_FOLDER = (
        "ID voor albummap"
    )
    SETTING_INCLUDE_EP = (
        "Inclusief singles en EP's"
    )
    SETTING_SAVE_COVERS = (
        "Bewaar covers"
    )
