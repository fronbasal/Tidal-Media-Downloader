...
class LangCzech(object):
    ...
    SETTING_ONLY_M4A = (
        "Konvertovat mp4 na m4a"
    )
    SETTING_ADD_EXPLICIT_TAG = (
        "Přidat označení explicity"
    )
    SETTING_ADD_HYPHEN = (
        "Místo mezer použít pomlčky"
    )
    SETTING_ADD_YEAR = (
        "Přidat rok před jméno složky"
    )
    SETTING_USE_TRACK_NUM = (
        "Přidat číslo skladby"
    )
    ...
    SETTING_ALBUMID_BEFORE_FOLDER = (
        "Id před složkou alba"
    )
    SETTING_INCLUDE_EP = (
        "Zahrnout singly & EP"
    )
    ...
    SETTING_USE_PLAYLIST_FOLDER = (
        "Používat složku playlistu"
    )
    SETTING_MULITHREAD_DOWNLOAD = (
        "Stahování více vlákny"
    )
    SETTING_ALBUM_FOLDER_FORMAT = (
        "Formát názvu složky alba"
    )
    SETTING_PLAYLIST_FOLDER_FORMAT = (
        "Playlist folder format"
    )
    SETTING_TRACK_FILE_FORMAT = (
        "Formát názvu souboru skladby"
    )
    SETTING_VIDEO_FILE_FORMAT = (
        "Formát názvu souboru videa"
    )
    SETTING_SHOW_PROGRESS = "Zobrazit indikátor stavu stahování"
    SETTING_SHOW_TRACKINFO = (
        "Zobrazit informace o skladbě"
    )
    SETTING_SAVE_ALBUMINFO = (
        "Uložit soubor AlbumInfo.txt"
    )
    SETTING_DOWNLOAD_VIDEOS = (
        "Download videos"
    )
    SETTING_ADD_LYRICS = (
        "Přidat texty skladeb"
    )
    SETTING_LYRICS_SERVER_PROXY = (
        "Server proxy pro texty skladeb"
    )
    ...
    SETTING_ADD_TYPE_FOLDER = (
        "Složky dle typu obsahu"
    )
    SETTING_DOWNLOAD_DELAY = (
        "Stahovat s časovou prodlevou"
    )
    ...
    CHOICE_EXIT = "Ukončit"
    CHOICE_LOGIN = (
        "Zkontrolovat přístupový token"
    )
    CHOICE_SETTINGS = "Nastavení"
    CHOICE_SET_ACCESS_TOKEN = (
        "Nastavit přístupový token"
    )
    CHOICE_DOWNLOAD_BY_URL = (
        "Stáhnout buď dle URL nebo ID"
    )
    ...
    MODEL_ALBUM_PROPERTY = (
        "ALBUM-VLASTNOSTI"
    )
    MODEL_TRACK_PROPERTY = (
        "SKLADBA-VLASTNOSTI"
    )
    MODEL_VIDEO_PROPERTY = (
        "VIDEO-VLASTNOSTI"
    )
    MODEL_ARTIST_PROPERTY = (
        "INTERPRET-VLASTNOSTI"
    )
    MODEL_PLAYLIST_PROPERTY = (
        "PLAYLIST-VLASTNOSTI"
    )
