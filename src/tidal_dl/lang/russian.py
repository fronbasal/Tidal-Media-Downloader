...
class LangRussian(object):
    ...
    SETTING_ONLY_M4A = (
        "Сохранять mp4 как m4a"
    )
    SETTING_ADD_EXPLICIT_TAG = (
        "Добавить тег Нецензурно"
    )
    SETTING_ADD_HYPHEN = (
        "Добавить дефис"
    )
    ...
    SETTING_ALBUMID_BEFORE_FOLDER = (
        "ID перед названием папки"
    )
    SETTING_INCLUDE_EP = (
        "Включить сингл и миньон"
    )
    SETTING_SAVE_COVERS = (
        "Добавлять обложку"
    )
    ...
    SETTING_SHOW_PROGRESS = (
        "Показывать процесс загрузки"
    )
    SETTING_SHOW_TRACKINFO = (
        "Show Track Info"
    )
    SETTING_SAVE_ALBUMINFO = (
        "Сохранять AlbumInfo.txt"
    )
    SETTING_DOWNLOAD_VIDEOS = (
        "Download videos"
    )
    SETTING_ADD_LYRICS = (
        "Добавлять текст песень"
    )
