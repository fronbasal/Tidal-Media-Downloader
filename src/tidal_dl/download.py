...
def downloadVideo(
    ...
    try:
        ...
        if m3u8content is None:
            Printf.err(
                f"DL Video[{video.title}] getM3u8 failed.{str(e)}"
            )
            return (
                False,
                f"GetM3u8 failed.{str(e)}",
            )
        ...
        if len(urls) <= 0:
            Printf.err(
                f"DL Video[{video.title}] getTsUrls failed.{str(e)}"
            )
            return (
                False,
                "GetTsUrls failed.{str(e)}",
            )
