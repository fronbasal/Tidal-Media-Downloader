...
class Settings(
    ...
    def read(self, path):
        self._path_ = path
        txt = tidal_dl.aigpy.file.getContent(
            self._path_
        )
        if len(txt) > 0:
            data = json.loads(txt)
        ...
        if (
            self.albumFolderFormat
            is None
        ):
            self.albumFolderFormat = self.getDefaultPathFormat(
                Type.Album
            )
        if self.trackFileFormat is None:
            self.trackFileFormat = self.getDefaultPathFormat(
                Type.Track
            )
        if (
            self.playlistFolderFormat
            is None
        ):
            self.playlistFolderFormat = self.getDefaultPathFormat(
                Type.Playlist
            )
        if self.videoFileFormat is None:
            self.videoFileFormat = self.getDefaultPathFormat(
                Type.Video
            )
    ...
    def save(self):
        ...
        data["audioQuality"] = (
            self.audioQuality.name
        )
        data["videoQuality"] = (
            self.videoQuality.name
        )
        txt = json.dumps(data)
