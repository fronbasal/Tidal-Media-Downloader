...
def start_artist(obj: Artist):
    albums = TIDAL_API.getArtistAlbums(
        obj.id, SETTINGS.includeEP
    )
...
def start_mix(obj: Mix):
    Printf.mix(obj)
    downloadTracks(
        obj.tracks, None, None
    )
    downloadVideos(
        obj.videos, None, None
    )
...
def changeQualitySettings():
    ...
    SETTINGS.audioQuality = AudioQuality(
        int(
            Printf.enterLimit(
                LANG.select.CHANGE_AUDIO_QUALITY,
    ...
    SETTINGS.videoQuality = VideoQuality(
        int(
            Printf.enterLimit(
                LANG.select.CHANGE_VIDEO_QUALITY,
                LANG.select.MSG_INPUT_ERR,
                [
                    "1080",
                    "720",
...
def changeSettings():
    Printf.settings()
    SETTINGS.showProgress = Printf.enterBool(
        LANG.select.CHANGE_SHOW_PROGRESS
    )
    SETTINGS.showTrackInfo = Printf.enterBool(
        LANG.select.CHANGE_SHOW_TRACKINFO
    )
    SETTINGS.checkExist = Printf.enterBool(
        LANG.select.CHANGE_CHECK_EXIST
    )
    SETTINGS.includeEP = Printf.enterBool(
        LANG.select.CHANGE_INCLUDE_EP
    )
    SETTINGS.saveCovers = Printf.enterBool(
        LANG.select.CHANGE_SAVE_COVERS
    )
    SETTINGS.saveAlbumInfo = Printf.enterBool(
        LANG.select.CHANGE_SAVE_ALBUM_INFO
    )
    SETTINGS.downloadVideos = Printf.enterBool(
        LANG.select.CHANGE_DOWNLOAD_VIDEOS
    )
    SETTINGS.lyricFile = Printf.enterBool(
        LANG.select.CHANGE_ADD_LRC_FILE
    )
    SETTINGS.multiThread = Printf.enterBool(
        LANG.select.CHANGE_MULITHREAD_DOWNLOAD
    )
    SETTINGS.usePlaylistFolder = Printf.enterBool(
        LANG.select.SETTING_USE_PLAYLIST_FOLDER
        + "('0'-No,'1'-Yes):"
    )
    SETTINGS.downloadDelay = Printf.enterBool(
        LANG.select.CHANGE_USE_DOWNLOAD_DELAY
    )
    SETTINGS.language = Printf.enter(
        LANG.select.CHANGE_LANGUAGE
        + "("
        + LANG.getLangChoicePrint()
        + "):"
    )
...
def loginByWeb():
    try:
        ...
        while (
            ...
            if (
                not TIDAL_API.checkAuthStatus()
            ):
                time.sleep(
                    TIDAL_API.key.authCheckInterval
                    + 1
                )
                continue

            Printf.success(
                LANG.select.MSG_VALID_ACCESSTOKEN.format(
                    __displayTime__(
                        int(
                            TIDAL_API.key.expiresIn
                        )
                    )
                )
            )
            ...
            TOKEN.userid = (
                TIDAL_API.key.userId
            )
            TOKEN.countryCode = (
                TIDAL_API.key.countryCode
            )
            TOKEN.accessToken = (
                TIDAL_API.key.accessToken
            )
            TOKEN.refreshToken = (
                TIDAL_API.key.refreshToken
            )
            TOKEN.expiresAfter = time.time() + int(
                TIDAL_API.key.expiresIn
            )
            TOKEN.save()
            return True
...
def loginByConfig(silent=False):
    try:
        ...
        if TIDAL_API.verifyAccessToken(
            TOKEN.accessToken
        ):
            if not silent:
                Printf.info(
        ...
        if not silent:
            Printf.info(
                LANG.select.MSG_INVALID_ACCESSTOKEN
            )

        if TIDAL_API.refreshAccessToken(
            TOKEN.refreshToken
        ):
            if not silent:
                Printf.success(
...
def loginByAccessToken():
    try:
        ...
        token = Printf.enter(
            "accessToken('0' go back):"
        )
        if token == "0":
            return
        TIDAL_API.loginByAccessToken(
            token, TOKEN.userid
        )
    except Exception as e:
        Printf.err(str(e))
    ...
    refreshToken = Printf.enter(
        "refreshToken('0' to skip):"
    )
    if refreshToken == "0":
        refreshToken = (
            TOKEN.refreshToken
