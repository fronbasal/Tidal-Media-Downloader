...
class TidalAPI(object):
    ...
    def getDeviceCode(self) -> str:
        ...
        self.key.deviceCode = result[
            "deviceCode"
        ]
        self.key.userCode = result[
            "userCode"
        ]
        self.key.verificationUrl = (
            result["verificationUri"]
        )
        self.key.authCheckTimeout = (
            result["expiresIn"]
        )
        self.key.authCheckInterval = (
            result["interval"]
        )
        return (
            "http://"
    ...
    def checkAuthStatus(self) -> bool:
        ...
        if (
            ...
            if (
                ...
                raise Exception(
                    "Error while checking for authorization. Trying again..."
                )

        # if auth is successful:
        self.key.userId = result[
            "user"
        ]["userId"]
        self.key.countryCode = result[
            "user"
        ]["countryCode"]
        self.key.accessToken = result[
            "access_token"
        ]
        self.key.refreshToken = result[
            "refresh_token"
        ]
        self.key.expiresIn = result[
            "expires_in"
        ]
        return True
    ...
    def getMix(self, id) -> Mix:
        mix = Mix()
        mix.id = id
        mix.tracks, mix.videos = (
            self.getItems(id, Type.Mix)
        )
        return None, mix
        ...
        for period in root.findall(
            ...
            for (
                ...
                for (
                    ...
                    content_type = adaptation_set.get(
                        "contentType"
                    )
                    if (
                        content_type
                    ...
                    # Also see example 9
                    seg_timeline = seg_template.find(
                        "SegmentTimeline"
                    )
                    if (
                        seg_timeline
    ...
    def getStreamUrl(
        ...
        data = self.__get__(
            f"tracks/{str(id)}/playbackinfopostpaywall",
            paras,
        )
        resp = tidal_dl.aigpy.model.dictToModel(
            data, StreamRespond()
        )
        ...
        raise Exception(
            "Can't get the streamUrl, type is "
            + resp.manifestMimeType
        )
    ...
    def getVideoStreamUrl(
        ...
        data = self.__get__(
            f"videos/{str(id)}/playbackinfopostpaywall",
            paras,
        )
        resp = tidal_dl.aigpy.model.dictToModel(
            data, StreamRespond()
        ...
        if (
            ...
            array = self.__getResolutionList__(
                manifest["urls"][0]
            )
