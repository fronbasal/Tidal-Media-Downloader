...
class LangRussian(object):
    ...
    CHANGE_AUDIO_QUALITY = (
        "Качество аудио('0'-Стандарт,'1'-Высокое,'2'-HiFi,'3'-MQA,'4'-Max):"
    )
    ...
    CHANGE_ADD_HYPHEN = (
        "Использовать дефис вместо пробела в имени трека('0'-Нет,'1'-Да):"
    )
    ...
    CHANGE_ARTIST_BEFORE_TITLE = (
        "Добавить имя артиста перед названием трека('0'-Нет,'1'-Да):"
    )
    CHANGE_INCLUDE_EP = "Включать синглы и миньоны в дискографию('0'-Нет'1'-Да):"
    CHANGE_ALBUMID_BEFORE_FOLDER = (
        "Добавить ID перед названием альбома('0'-Нет,'1'-Да):"
    )
