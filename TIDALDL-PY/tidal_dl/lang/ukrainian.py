...
class LangUkrainian(object):
    ...
    CHANGE_AUDIO_QUALITY = (
        "Якість аудіо('0'-Звичайна,'1'-Висока,'2'-HiFi,'3'-MQA,'4'-Max):"
    )
    ...
    CHANGE_ADD_HYPHEN = (
        "Використати дефіс замість пробілів в іменах файлів('0'-Ні,'1'-Так):"
    )
    CHANGE_ADD_YEAR = "Додавати рік до назв тек альбомів('0'-Ні,'1'-Так):"
    CHANGE_USE_TRACK_NUM = (
        "Додавати номер доріжки перед назвами файлів('0'-Ні,'1'-Так):"
    )
    CHANGE_CHECK_EXIST = (
        "Перевіряти наявний файл перед завантаженням треку('0'-Ні,'1'-Так):"
    )
    CHANGE_ARTIST_BEFORE_TITLE = (
        "Додати ім’я виконавця перед заголовком треку('0'-Ні,'1'-Так):"
    )
    ...
    CHANGE_ALBUMID_BEFORE_FOLDER = (
        "Додати ідентифікатор перед текою альбому('0'-Ні,'1'-Так):"
    )
    ...
    CHANGE_LYRICS_SERVER_PROXY = (
        "Проксі для сервера з текстами пісень('0'-не змінювати):"
    )
    CHANGE_ADD_LRC_FILE = (
        "Зберігати тексти пісень з відмітками часу в .lrc файл('0'-Ні,'1'-Так):"
    )
    CHANGE_ADD_TYPE_FOLDER = (
        "Додавати теку-тип,наприклад Альбом/Відео/Плейлист('0'-Ні,'1'-Так):"
    )
    ...
    MSG_INVALID_ACCESSTOKEN = (
        "Термін дії AccessToken'а закінчився. Пробуємо оновити його."
    )
