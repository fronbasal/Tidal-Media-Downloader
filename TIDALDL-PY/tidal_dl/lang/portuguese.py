...
class LangPortuguese(object):
    ...
    CHANGE_AUDIO_QUALITY = (
        "Qualidade De Áudio('0'-Normal,'1'-Alta,'2'-HiFi,'3'-Master,'4'-Max):"
    )
    ...
    CHANGE_ADD_EXPLICIT_TAG = (
        "Adicionar Etiqueta Explícito para os nomes dos arquivos('0'-Não,'1'-Sim):"
    )
    CHANGE_ADD_HYPHEN = (
        "Usar hífen em vez de espaços nos nomes dos arquivos('0'-Não,'1'-Sim):"
    )
    CHANGE_ADD_YEAR = "Adicionar Ano aos nomes das pastas dos álbums('0'-Não,'1'-Sim):"
    CHANGE_USE_TRACK_NUM = (
        "Adicionar número da faixa antes do nome do arquivo('0'-Não,'1'-Sim):"
    )
    CHANGE_CHECK_EXIST = (
        "Verificar existência do arquivo antes de baixar a faixa('0'-Não,'1'-Sim):"
    )
    CHANGE_ARTIST_BEFORE_TITLE = (
        "Adicionar o nome do artista antes do título da faixa('0'-Não,'1'-Sim):"
    )
    CHANGE_INCLUDE_EP = (
        "Incluir singles e EPs quando baixar álbums de um artista('0'-Não,'1'-Sim):"
    )
    CHANGE_ALBUMID_BEFORE_FOLDER = (
        "Adicionar id antes no nome da pasta do álbum('0'-Não,'1'-Sim):"
    )
    ...
    CHANGE_ADD_LRC_FILE = (
        "Salvar arquivo .lrc de letras cronometradas ('0'-Não,'1'-Sim):"
    )
