...
class LangFrench(object):
    ...
    CHANGE_AUDIO_QUALITY = (
        "Qualité audio('0'-Normal,'1'-High,'2'-HiFi,'3'-Master,'4'-Max):"
    )
    ...
    CHANGE_ADD_YEAR = (
        "Ajouter l'année aux noms des dossiers des albums('0'-Non,'1'-Oui):"
    )
    CHANGE_USE_TRACK_NUM = (
        "Ajouter le numéro de piste avant le nom des fichiers('0'-Non,'1'-Oui):"
    )
    CHANGE_CHECK_EXIST = (
        "Vérifier l'existence du fichier avant le téléchargement('0'-Non,'1'-Oui):"
    )
    CHANGE_ARTIST_BEFORE_TITLE = (
        "Ajouter le nom de l'artiste avant le titre de la piste('0'-Non,'1'-Oui):"
    )
    ...
    CHANGE_ALBUMID_BEFORE_FOLDER = (
        "Ajouter un identifiant avant le dossier album('0'-Non,'1'-Oui):"
    )
    ...
    CHANGE_ADD_LRC_FILE = (
        "Enregistrer les paroles synchronisées (fichier.lrc) ('0'-Non,'1'-Oui):"
    )
    CHANGE_ADD_TYPE_FOLDER = (
        "Ajouter un dossier de type,Ex: Album/Video/Playlist('0'-Non,'1'-Oui):"
    )
    ...
    MSG_INVALID_ACCESSTOKEN = (
        "Token d'accès expiré. Tentative de renouvellement automatique."
    )
